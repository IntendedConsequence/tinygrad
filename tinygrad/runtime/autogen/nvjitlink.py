...
dll = c.DLL('nvjitlink', 'nvJitLink' if not WIN else (glob.glob(os.path.join(os.environ.get('CUDA_PATH', ''), 'bin', 'nvJitLink_*.dll'))+[''])[0], [f'/{pre}/cuda/targets/{tgt}/lib' for pre in ['opt', 'usr/local'] for tgt in [sysconfig.get_config_vars().get("MULTIARCH", "").rsplit("-", 1)[0], 'sbsa-linux']])
