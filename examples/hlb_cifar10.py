...
def train_cifar():
  ...
  # cutmix always, select between arange and randperm via variable 0-1 inside augmentations
  @TinyJit
  def augmentations(X:Tensor, Y:Tensor, do_cutmix):
    ...
    arange = Tensor.arange(X.shape[0], device=X.device) # We reuse perms for cutmix, because they are expensive to generate
